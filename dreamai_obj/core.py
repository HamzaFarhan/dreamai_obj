...
# %% auto 0
__all__ = ['obj_model', 'obj_detect', 'box_overlap', 'distance_to_camera', 'detect_obstacles_3', 'blur_faces', 'blur_faces_video',
           'save_video']
...
def blur_faces(model, img, conf=0.3, h_div=3):
    if path_or_str(img):
        img = rgb_read(img)
    boxes, cats = obj_detect(model, img, conf=conf)
    for box,cat in zip(boxes, cats):
...
def blur_faces_video(model, video, conf=0.3, h_div=2):
    ...
    frames = [blur_faces(model, frame.copy(), conf=conf, h_div=h_div) for frame in video.iter_frames()]
